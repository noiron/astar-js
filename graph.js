...
GraphSearch.prototype.initialize = function() {
    ...
    this.$cells.each(function() {
        var rand = Math.floor(Math.random() / self.opts.wallFrequency);
        if (rand == 0) {
            $(this).addClass(css.wall);
        }
    });
...
GraphSearch.prototype.animatePath = function(path) {
    ...
    this.$graph.find("." + css.finish).removeClass(css.start).addClass(css.start);
};
...
GraphNode.prototype.isAt = function() {
